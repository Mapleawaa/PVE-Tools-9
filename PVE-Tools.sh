...
def run_smartctl(device, args=""):
    ...
    cmd = f"timeout 2 smartctl {device} {args} -n standby -a -j 2>/dev/null"
    try:
    ...
    for sd in $(ls /dev/sd[a-z] 2> /dev/null); do
...

        {
            ...
            if (\$cache_data =~ /^\s*\{/) {
                # 使用 JSON 解析替代正则提取
                eval {
                    my \$cache_obj = JSON::PP::decode_json(\$cache_data);
                    my \$entry = \$cache_obj->{"sd$sdi"};
                    if (\$entry && ref(\$entry) eq 'HASH') {
                        \$sd_data = JSON::PP::encode_json(\$entry);
                    }
                };
            }
            FALLBACK_sd$sdi:
    ...
    while read -r line; do
        ...
        {   # 块作用域，避免变量重复声明
            ...
            if (\$cache_data =~ /^\s*\{/) {
                ...
                eval {
                    my \$cache_obj = JSON::PP::decode_json(\$cache_data);
                    my \$entry = \$cache_obj->{"raid$raidi"};
                    if (\$entry && ref(\$entry) eq 'HASH') {
                        \$raid_data = JSON::PP::encode_json(\$entry);
                    }
                };
            }
            FALLBACK_raid$raidi:
