...
network_can_access_internet() {
    local test_urls=(
        "https://pve.u3u.icu/VERSION"
        "${GITHUB_MIRROR_PREFIX}raw.githubusercontent.com/Mapleawaa/PVE-Tools-9/main/VERSION"
        "$VERSION_FILE_URL"
    )
    for url in "${test_urls[@]}"; do
        if command -v curl >/dev/null 2>&1; then
            if curl -fsSL --connect-timeout 5 --max-time 8 "$url" >/dev/null 2>&1; then
                return 0
            fi
        elif command -v wget >/dev/null 2>&1; then
            if wget -q --timeout=8 -O - "$url" >/dev/null 2>&1; then
                return 0
            fi
        fi
    done
...
disable_ups_service() {
    ...
    elif systemctl list-unit-files 2>/dev/null | grep -q '^nut-server\.service'; then
        systemctl stop nut-server >/dev/null 2>&1 || true
        systemctl stop nut-client >/dev/null 2>&1 || true
        systemctl disable nut-server >/dev/null 2>&1 || true
        systemctl disable nut-client >/dev/null 2>&1 || true
        log_success "已执行 UPS 服务关闭: systemctl stop/disable nut-server nut-client"
    else
        log_info "未检测到 apcupsd 或 nut-server，跳过 UPS 服务管理"
    fi
    return 0
}
...
enable_ups_service() {
    ...
    if systemctl list-unit-files 2>/dev/null | grep -q '^apcupsd\.service'; then
        ...
        systemctl start nut-server >/dev/null 2>&1 || true
        return 0
    fi
    return 1
}
...
cpu_add() {
    ...
    log_step "开始安装所需工具..."
    # 输入需要安装的软件包 (添加 hdparm 用于 SATA 硬盘休眠检测, apcupsd/nut for UPS support)
    ...
    echo "  1) apcupsd  (使用 apcaccess 采集数据)"
    echo "  2) NUT      (使用 upsc 采集数据)"
    echo "  0) 不启用"
    read -p "请选择 UPS 类型 [0-2] (默认0): " ups_type_choice
    echo
    case "${ups_type_choice:-0}" in
        1)
            enable_ups=true
            ups_backend=apcupsd
            log_success "已选择 apcupsd UPS 监控"
            if ! command -v apcaccess >/dev/null 2>&1; then
                log_step "正在安装 apcupsd..."
                if ! apt-get install -y apcupsd; then
                    log_warn "apcupsd 安装失败，请手动执行: apt-get install -y apcupsd"
                else
                    log_success "apcupsd 安装完成"
                fi
            else
                log_info "检测到 apcupsd 已安装，跳过安装步骤"
            fi
        ...
        2)
            enable_ups=true
            ups_backend=nut
            log_success "已选择 NUT UPS 监控"
            if ! command -v upsc >/dev/null 2>&1; then
                log_step "正在安装 nut..."
                if ! apt-get install -y nut; then
                    log_warn "nut 安装失败，请手动执行: apt-get install -y nut"
                else
                    log_success "nut 安装完成"
                fi
            else
                log_info "检测到 NUT 已安装，跳过安装步骤"
            fi
            # 自动检测已配置的 UPS 名称
            local detected_ups_name=""
            if [ -f /etc/nut/ups.conf ]; then
                detected_ups_name=$(grep -oP '^\[\K[^\]]+' /etc/nut/ups.conf | head -1)
            fi
            if [ -n "$detected_ups_name" ]; then
                log_info "检测到已配置的 UPS 名称: ${detected_ups_name}"
                read -p "使用此名称？直接回车确认，或输入新名称: " input_ups_name
                nut_ups_name="${input_ups_name:-$detected_ups_name}"
            else
                read -p "请输入 UPS 名称（/etc/nut/ups.conf 中 [...] 内的名称，默认 ups）: " input_ups_name
                nut_ups_name="${input_ups_name:-ups}"
            fi
            log_success "将使用 UPS 名称: ${nut_ups_name}"
            # 验证 upsc 能否取到数据
            if command -v upsc >/dev/null 2>&1; then
                log_step "验证 upsc ${nut_ups_name} 连通性..."
                if upsc "${nut_ups_name}" >/dev/null 2>&1; then
                    log_success "upsc ${nut_ups_name} 连通正常"
                else
                    log_warn "upsc ${nut_ups_name} 无法获取数据，请确认 NUT 服务已运行且 UPS 名称正确"
                    log_tips "可手动测试: upsc ${nut_ups_name}"
                fi
            fi
            if systemctl list-unit-files 2>/dev/null | grep -q '^nut-server\.service'; then
                if enable_ups_service; then
                    log_info "已启用并启动 nut-server 服务"
                else
                    log_warn "未能自动启用 nut-server 服务，UPS 信息可能为空"
                fi
            else
                log_tips "NUT 需要手动配置 /etc/nut/ups.conf 和 /etc/nut/upsd.conf 后再启用服务"
            fi
            ;;
        *)
            enable_ups=false
...
EOF
    ...
    if [ "$enable_ups" = true ]; then
        if [ "$ups_backend" = "nut" ]; then
            cat >> $tmpf << EOF
        \$res->{ups_status} = \`upsc ${nut_ups_name} 2>/dev/null\`;
        \$res->{ups_backend} = 'nut';
EOF
        else
            cat >> $tmpf << 'EOF'
        $res->{ups_status} = `apcaccess status`;
        $res->{ups_backend} = 'apcupsd';
EOF
        fi
    fi
...

    {
        ...
        renderer: function(value) {
            if (!value || value.length === 0) {
                return '提示: 未检测到 UPS 或 UPS 服务未运行';
            }

            try {
                // 判断后端类型：NUT 输出格式为 key: value，apcupsd 为 KEY      : value
                const isNut = /^[a-z][\w.]+\s*:/m.test(value);
                ...
                if (isNut) {
                    ...
                    const MODEL    = kv['ups.model'] || kv['device.model'] || '未知型号';
                    const MFGR     = kv['ups.mfr'] || kv['device.mfr'] || '';
                    const STATUS   = kv['ups.status'] || 'UNKNOWN';
                    const BCHARGE  = kv['battery.charge'] || '-';
                    const TIMELEFT = kv['battery.runtime'] ? (parseFloat(kv['battery.runtime']) / 60).toFixed(1) : '-';
                    const VOLTAGE  = kv['output.voltage'] || kv['input.voltage'] || '-';
                    const LOADPCT  = kv['ups.load'] || '-';
                    const NOMPOWER = kv['ups.realpower.nominal'] ? parseFloat(kv['ups.realpower.nominal']) : NaN;

                    let powerStatusText = '';
                    if (STATUS.includes('OL') && !STATUS.includes('OB')) {
                    ...
                    if (!isNaN(NOMPOWER) && NOMPOWER > 0) {
                        totalPowerText = NOMPOWER.toFixed(0) + ' W';
                        const lp = parseFloat(LOADPCT);
                        if (!isNaN(lp)) {
                            currentPowerText = (NOMPOWER * lp / 100).toFixed(0) + ' W';
                        }
                    }

                    const displayModel = MFGR ? MFGR + ' ' + MODEL : MODEL;
                    return `${displayModel} | ${powerStatusText}<br>
                            电量: ${BCHARGE} % | 剩余供电时间: ${TIMELEFT} 分钟<br>
                            电压: ${VOLTAGE} V | 负载: ${LOADPCT} %<br>
                            额定功率: ${totalPowerText} | 估算当前功率: ${currentPowerText}`;
                } else {
                    // apcupsd (apcaccess) 解析
                    ...
                    let powerStatusText = '';
                    switch (STATUS) {
                        case 'ONLINE':  powerStatusText = '市电供电正常'; break;
                        case 'ONBATT':  powerStatusText = '电池供电中（市电中断）'; break;
                        case 'CHRG':    powerStatusText = '电池充电中'; break;
                        case 'DISCHRG': powerStatusText = '电池放电中'; break;
                        default:        powerStatusText = '状态: ' + STATUS; break;
                    }
                    ...
                    if (!isNaN(NOMPOWER) && NOMPOWER > 0) {
                        totalPowerText = NOMPOWER.toFixed(0) + ' W';
                        if (!isNaN(LOADPCT)) {
                            currentPowerText = (NOMPOWER * LOADPCT / 100).toFixed(0) + ' W';
                        }
                    }
...
cpu_del() {
    ...
    if systemctl list-unit-files 2>/dev/null | grep -q '^apcupsd\.service'; then
        ...
        ups_service="nut-server"
    fi
    if [ -n "$ups_service" ]; then
        echo -n "检测到 ${ups_service} 服务，是否同时停用？(y/N): "
        read -n 1 -r
        echo
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            disable_ups_service
...
temp_monitoring_menu() {
    while true; do
        ...
        local ups_backend_label="未安装"
        if systemctl list-unit-files 2>/dev/null | grep -q '^apcupsd\.service'; then
            ups_backend_label="apcupsd"
        elif systemctl list-unit-files 2>/dev/null | grep -q '^nut-server\.service'; then
            ups_backend_label="NUT"
        fi
        show_menu_option "4" "UPS 服务管理 ${CYAN}(${ups_backend_label})${NC}"
        echo "${UI_DIVIDER}"
        show_menu_option "0" "返回上级菜单"
        show_menu_footer
        echo
        ...
        case $temp_choice in
            ...
            4)
                if ! command -v systemctl >/dev/null 2>&1; then
                    log_warn "系统不支持 systemctl，无法管理 UPS 服务"
                elif systemctl list-unit-files 2>/dev/null | grep -q '^apcupsd\.service'; then
                    ...
                    echo "当前 UPS 服务 (apcupsd): active=${active_state}, enabled=${enabled_state}"
                    ...
                    local active_state enabled_state
                    active_state=$(systemctl is-active nut-server 2>/dev/null || echo unknown)
                    enabled_state=$(systemctl is-enabled nut-server 2>/dev/null || echo unknown)
                    echo "当前 UPS 服务 (nut-server): active=${active_state}, enabled=${enabled_state}"
                    echo "1) 禁用 UPS 服务（stop + disable）"
                    echo "2) 启用 UPS 服务（enable + start）"
                    echo "0) 返回"
                    read -p "请选择 [0-2]: " ups_choice
                    ...
                    log_warn "未检测到 apcupsd 或 nut-server，无法管理 UPS 服务"
                fi
                ;;
            0)
                break
...
custom_temp_monitoring() {
    ...
    # options[9]="UPS 信息 (支持 apcupsd - apcaccess 或 NUT - upsc)"
